...
parser.add_argument('--model', '-m', type=str, default='vgg19', help='choose model')
...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/{}_ckpt.pth'.format(args.model))
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(150), gamma=0.1)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/{}_ckpt.pth'.format(args.model))
        best_acc = acc
